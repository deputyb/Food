...
<body>
	...
	if (!isUpdate) {
		if (WCMMode.fromRequest(request) == WCMMode.DESIGN) {
			%><fmt:message key="You can edit this form using the"/> <a target="_new" href="<%= dlgPath %>.html"><fmt:message key="dialog editor"/></a><br /></body><%
			return;
		}
		...
		if (contentPath.length() == 0 || templatePath.length() == 0) {
			%><fmt:message key="scaffoldingError"/>.<br /></body><%
			return;
		} else {
			%><fmt:message key="createPages"/> <a href="<%= contentPath %>.html"><%= contentPath %></a><ul id="linklist"></ul><%
		}
	}
	%><br />
